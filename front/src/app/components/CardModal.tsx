...
export default function CardModal({onClick}:any) {
  ...
  return (
    <Stack className="bg-black bg-opacity-70 w-full h-full fixed top-0  z-10 flex justify-center items-center">
      ...
      <Typography
            sx={{ fontSize: "16px", fontWeight: "400", cursor: 'pointer', textAlign: "end" }}
            onClick={onClick} >
  <ArrowBackIosIcon/>
          </Typography>
            ...
            foodData.map((foodItem, index) => (
              <Card key={index}>
                <CardContent sx={{display:'flex'}}>
                  <img className=" w-[245px] h-[150px]" src={foodItem.selectedFood.image} />
                ...
                <Typography>{foodItem.selectedFood.name}  </Typography>
                    ...
                    <Box display={'flex'}>
                  <Button className=" bg-[#18BA51] text-white " sx={{width:'10px'}} onClick={() => decreaseCount(index)}>-</Button>
        ...
        <Card className="w-full " sx={{ marginBottom: '20px', display: "flex", justifyContent: "center" }} >
          <CardContent sx={{ display: 'flex' }}>
            <Box>
              <Typography >
                Нийт төлөх дүн
